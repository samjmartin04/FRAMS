...
while True:
    response = input("\nA  Add to existing schedule\nC  Check Schedule\nG  Generate Schedule\nO  Optimise Schedule\nS  Settings\n\nChoose option: ")

    if response == "":
        break
    elif response.casefold()[:1] == "a":
        print("")
        ...
        if location_name == "":
            location_name = f"{old_schedule_location[:-4]}_extended.txt"
        
        location = check_location(location_name)

        program_start = int(time.time())
        match_schedule = generate_schedule(number_of_teams, number_of_appearances, optimisation_level=speed_mode, silent=True, match_schedule=match_schedule)
